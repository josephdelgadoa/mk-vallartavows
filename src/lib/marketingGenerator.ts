...
export async function generateMarketingContent(params: GenerationParams) {
    if (!GOOGLE_API_KEY) {
    ...
    try {
        ...
        cleanText = cleanText
            ...
            .replace(/\\([^"\\/bfnrtu])/g, "$1"); // <--- HEAL INVALID ESCAPES
